...
- crossterm 0.29 is pinned to `gronke/fork-crossterm` (`[patch.crates-io]`) so the crate compiles on neutral targets: upstream master plus additive `cfg(not(any(unix, windows)))` stubs, the branch behind crossterm-rs/crossterm#1066.
  Terminal control reports `Unsupported` at runtime and nothing ever reads a TTY.
...
- A manifest entry may declare a word pool: the page renders it as an editable textarea and answers both panes' `query-changed` from it, the standalone host role with the same `InMemorySource` semantics as the form's live pool (ADR 0014).
...
- The worker is the module-resolution boundary: the page's importmap must keep `lit` resolving to the real lit for the browser pane, and import maps do not reach workers at all.
  The build therefore emits a worker module tree: the runtime compiled to files (the same per-module TypeScript pass the Boa host bakes) beside the vendored package with its bare `lit*` imports rewritten to relative paths.
