...
The `uic1` head is the version marker and the positive identifier — any reader (a paste box, a clipboard watch) tells a uic:p2p credential for certain instead of guessing from structure, and the digit bumps when the layout changes — while structural validation guards the rest: length prefixes must fit, the setup byte and address tags are constrained, every byte must be consumed and at least one candidate be present, and in a link the fragment position is the discriminator.
...
- `payload_role`/`payloadRole` classify by decoding — a missing `uic1` head, then the structural checks (bad base64, unfitting lengths, alien setup and tag bytes, trailing bytes), make random text fail reliably — which is what the camera scanner and the paste guards lean on.
