...
The pairing UI is one shared component set: `<pair-panel>`, `<qr-code>` and the terminal's `<p2p-deck>`, baked into `@gronke/lit-todo` and loaded by both hosts.
Only the transport differs per host: the browser's `pair-wizard` owns the swap and the camera, the terminal's native Rust peer drives the same elements from the demo binary (ADR 0028).
...
Only `idle` and `invite` render a body: `idle` a lone "create an invite", `invite` the three-step wizard `step` walks (start, acknowledge, connect), the live status riding inside the reachable card.
Every other mode speaks through the status line, the badge and the action/reset buttons.
...
The wizard's reactive state speaks the panel's own vocabulary, so its render is a plain pass-through.
The terminal loop mirrors the session machine's `PanelState` onto the mounted panel with `set_prop` and polls `command` after each click.
...
Both hosts paint it black on white explicitly (a camera wants dark modules on a light ground whatever the theme): the browser's SVG on its own white card, the widget with explicit cell colors.
The half-block renderer (`qrcode` crate, Dense1x2) is one `render_qr` function shared by the widget and the live-mode join pane.

`<p2p-deck>` is the terminal's arrangement, pairing-first: the pairing panel is the boot screen, the `<status-navbar>` bar and the todo card staying `hidden` until a wire stands.
The todo card and the panel stack in one column and the QR docks to their right, wrapping below the panes under about 200 columns: real taffy flexbox from the deck's own stylesheet (a 111ch stack width as the flex basis beside the QR's ~87 intrinsic columns and the 2ch gap), no host rect math.
...
The panel's `static styles` are the terminal-only layer, hiding the copy-link and the inline QR (the deck docks its own beside the panes) while the mapped Bootstrap subset draws the card and the badge (ADR 0021); the browser page's CSS hides the link text instead.
...
The pairing UI existed in per-host copies, while the transport is the only part that genuinely cannot be shared: Boa has no WebRTC and no camera.
The mocked terminal lit has no `Event`/`CustomEvent`/`dispatchEvent`, and `JsHost` exposes only `set_prop`/`prop_json`, so a shared component cannot signal intent by dispatching an event.
The `command` property is the channel that works everywhere, and the browser event is a convenience on top of the same helper.
...
A structural re-render (the mode body swapping, a label button appearing or vanishing) tears the focused node down and the browser drops focus to `body`, knocking a keyboard user back to the top of the page.
The restoration keeps the keyboard walk in place.
...
- The deck's breakpoint is flex-basis arithmetic (the stack's `width` beside the QR's intrinsic columns), not a media query: `uic_css` maps `display: flex`, `flex-wrap`, `min-width` and `ch` lengths onto taffy, but has no `@media`/container conditions and no `flex-basis` property; those are the recorded follow-ups, issue #98.
- The camera stays browser-only: the terminal answers `scan` with nothing, and `canScan` lights the scan button only where `BarcodeDetector` exists.
  Both hosts now have a clipboard but drive it differently: the browser a 🔗 copy-link and a `canPaste` paste button, the terminal a background watch that auto-continues a pairing step when a peer credential lands.
  The terminal still answers `copy-link` with nothing, the link being selectable text.
