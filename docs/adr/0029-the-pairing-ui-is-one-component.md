...
Only `idle` and `invite` render a body — `idle` a lone "create an invite", `invite` the three-step wizard `step` walks (start, acknowledge, connect), the live status riding inside the reachable card — while every other mode speaks through the status line, the badge and the action/reset buttons.
...
`<p2p-deck>` is the terminal's arrangement, pairing-first: the pairing panel is the boot screen, the `<status-navbar>` bar and the todo card staying `hidden` until a wire stands. The todo card and the panel stack in one column and the QR docks to their right, wrapping below the panes under about 200 columns — real taffy flexbox from the deck's own stylesheet (a 111ch stack width as the flex basis beside the QR's ~87 intrinsic columns and the 2ch gap), no host rect math.
...
- The camera stays browser-only: the terminal answers `scan` with nothing, and `canScan` lights the scan button only where `BarcodeDetector` exists. Both hosts now have a clipboard but drive it differently — the browser a 🔗 copy-link and a `canPaste` paste button, the terminal a background watch that auto-continues a pairing step when a peer credential lands — while the terminal still answers `copy-link` with nothing, the link being selectable text.
