...
| `uic_tui` | The terminal runtime: components mount on the document (`dom/host`, ADR 0011), composites route children and events (`dom/composite`), holes resolve against the store (`dom/resolve`), one `WidgetAdapter` per rat widget (`dom/widget/`), taffy layout and ratatui paint (`dom/layout`, `dom/render`), and `App` hosts it all (ADR 0012). |
...
| `ui_components` | The catalog: the input components and the demo composition (`demo/app_root`), each a `.rs` definition beside its `.mhtml` template and `.impl.ts` twin. |
