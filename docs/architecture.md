...
| `uic_sync` | State sync as a reusable artifact (ADR 0024): the tagged structured-clone codec, the string-payload `Wire` seam (WebSocket, RTCDataChannel, BroadcastChannel), root-component attachment, compact-SDP QR pairing, and the `@schuhkarton/uic-sync` npm tree. |
