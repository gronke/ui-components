...
Each component module co-locates its Rust definition (`date.rs`), its lit-flavored template (`date.html`), its stylesheet (`date.scss`), and — for behavior the browser cannot derive — its web partial (`date.impl.ts`).
