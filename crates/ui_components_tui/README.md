...
Drift stays guarded structurally rather than by co-location: a twin keeps its mirrored path, registers the same `data-tui` string kind through `uic_tui`'s `inventory` `WidgetRegistration`, and runs under the same TestBackend suites.
