...
The dialect keeps what cells can express: display, box spacing, flex, sizes, colors (ANSI palette, 24-bit, `Highlight` as reverse video), text styling, `content` with right-angle `transform: rotate(...)`, and custom properties with `var()` and additive `calc()`.
Everything else drops into a report.
...
Rules targeting `::before`/`::after` resolve in a pseudo pass per element, inheriting from the owner.
An element's resolved entry carries its own style plus the pseudo styles whose cascade produced `content`.
