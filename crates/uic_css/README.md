...
The dialect keeps what cells can express — display, box spacing, flex, sizes, colors (ANSI palette, 24-bit, `Highlight` as reverse video), text styling, `content`, custom properties with `var()` and additive `calc()` — and drops everything else into a report.
