...
Browser host for the terminal runtime: the `uic_tui` stack compiled to WebAssembly, rendering
frames as ANSI for xterm.js and feeding DOM keyboard and pointer events back in.
...
The same host operations back the Boa natives on real terminals (`uic_js`) — one body per
operation, two thin wrappers.
