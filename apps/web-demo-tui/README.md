...
`uic_tui_web`'s `TuiSession` is a wasm-bindgen export that rides along into this cdylib.
`link_catalog` anchors the host and the catalog into the bundle so the linker keeps their inventory constructors, which `TuiSession::new`'s `__wasm_call_ctors` then runs.
