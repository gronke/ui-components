...
**`/p2p` — no server carries the state, and pairing comes first.** The page opens on the pairing screen alone; the todo list appears only once a wire stands, under a `<status-navbar>` that wears the connection badge, this side's address and a disconnect control.
Disconnect closes the wire deliberately and returns to a fresh invite — no reload — while the remote side keeps its list on screen with a red badge and the same way back; the rows persist through localStorage on both ends either way.
...
Opening a link tells the wizard which step to land on: a fresh invite (the peer is initiating) moves to acknowledge and re-presents the link as the reply the peer must open, while a reply answering your own invite skips straight to connecting — the digest riding a reply link (`link_reply`) is what tells them apart, and a reply for some other invite is refused rather than dialed.
Step three connects and retries what can honestly resume: a fresh-invite attempt mints a new swap and a fresh reply link, bounded; a reply-to-ours failure can't resume — the peer's copy of your payload went stale — so it renews a plain invite and says so.
A credential on the clipboard continues the step on its own: the browser reads it behind its permission prompt (a "paste from clipboard" button and a read on window focus), the terminal watches the system clipboard under `--clipboard` (opt-in) through the mocked DOM's `navigator.clipboard` — the same one JS reads — with a headless session leaving it inert.
A `uic1` magic head on every payload (`crates/uic_sync`) makes "is this ours?" a certain yes or no, so a stray copy is ignored rather than dialed; a genuinely different credential arriving mid-pairing asks first (a native `confirm` in the browser, a host-painted prompt in the terminal) before it drops the current attempt.
...
`cargo run -p uic_lit_demo -- p2p --serve` hosts the pairing page from the same process and points its invites at this machine, so one command stands the demo up on a LAN — the QR and link open the page this peer serves, and the terminal pairs straight back over WebRTC (the server only hands out the page bytes; no state crosses it).
The panel and the navbar are presentation only, driven by properties the host sets and signalling button intent back through a `command` property the terminal loop polls (Boa has no events); the transport differs per host — the browser's `pair-wizard` owns the swap and the clipboard, the terminal's native `webrtc-rs` peer carries the codec in Rust (`uic_sync::pair`, one byte contract with the page).
...
- `p2p --clipboard` opts into watching the system clipboard (off by default — reading a user's clipboard is not a default), which also exposes it to the page as `navigator.clipboard`. A pasted or scanned link always pairs regardless.
